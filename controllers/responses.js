...
exports.Post = async function(ctx) {
  ...
  ctx.body = success(await Responses.create(body));
}
...
exports.Del = async function(ctx) {
  ...
  let deleteCount = await Responses.destroy({
    where: { id }
  });
  if (!deleteCount) {
...
exports.Put = async function(ctx) {
  ...
  let [ affectedCount ] = await Responses.update(body, {
    where: { id }
  });
  if (!affectedCount) {
    throw ErrorCode.NotFoundError('id = ' + id);
...
exports.Get = async function(ctx) {
  ...
  let id = ctx.params.id;
  if (id) {
    results = await Responses.findById(id);
    if (!results) {
      throw ErrorCode.NotFoundError('id = ' + id);
    }
  } else {
    results = await Responses.findAll();
  }
